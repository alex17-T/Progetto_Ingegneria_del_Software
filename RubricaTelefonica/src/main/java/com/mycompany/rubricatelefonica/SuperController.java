/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.mycompany.rubricatelefonica;

import java.util.ArrayList;
import javafx.collections.FXCollections;
import javafx.collections.ObservableList;
import javafx.scene.control.TableColumn;
import javafx.scene.control.TableView;

/**
 *
 * @author Giovanni
 */
public class SuperController {
    public static TableView<Contatto> tab = new TableView();
    public static TableColumn<Contatto, String> colNome;
    public static TableColumn<Contatto, String> colCognome;
    public static TableColumn<Contatto, String> colNumTel;
    public static TableColumn<Contatto, String> colEmail;
    public static ObservableList<Contatto> lista = FXCollections.observableArrayList();
    
    
    
    
    
    /*public SuperController(TableView tableView, TableColumn colNome, TableColumn colCognome, ObservableList<Contatto> lista) {
        this.tab = tableView;
        this.colNome = colNome;
        this.colCognome = colCognome;
        this.lista = lista;
        /*this.colNumTel = colNumTel;
        this.colEmail = colEmail;*/
    //}
    

}
